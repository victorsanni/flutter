...
const Duration _kThemeChangeDuration = Duration(milliseconds: 200);
...
class RawCircleAvatar extends StatelessWidget {
  ...
  /// 20 when the [RawCircleAvatar] is in an unconstrained environment will cause
  /// the avatar to animate from a 20 pixel diameter to a 40 pixel diameter.
  /// However, if the [minRadius] is 40 and the [RawCircleAvatar] has a parent
  ...
  /// 20 when the [RawCircleAvatar] is in an unconstrained environment will cause
  /// the avatar to animate from a 20 pixel diameter to a 40 pixel diameter.
  /// However, if the [maxRadius] is 40 and the [RawCircleAvatar] has a parent
  /// [SizedBox] whose size changes instantaneously from 20 pixels to 40 pixels,
  /// the size will snap to 40 pixels instantly.
